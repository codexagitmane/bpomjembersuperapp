...
export default function PresensiPage() {
  ...
  const loadData = useCallback(async () => {
    const [kantorRes, hariIniRes, riwayatRes, wfhRes] = await Promise.all([
  ...
  // Shift keamanan lintas hari: kalau ada shift kemarin yg belum checkout,
  // aksi berikutnya adalah check-out shift tersebut.
  ...
  // Jarak client-side hanya untuk feedback UI; validasi final tetap di server.
  const jarakTerdekat =
    coords && kantor
      ? Math.min(
  ...
  const dalamRadius =
    jarakAcuan !== null && kantor ? jarakAcuan <= kantor.radius_meter : null;
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setSubmitSuccess(
        aksi === "check-in" ? "Presensi masuk berhasil dicatat!" : "Presensi pulang berhasil dicatat!"
      );
      setSelfie(null);
      await loadData();
    } catch (err) {
    ...
    <AppShell>
      <div className="mx-auto max-w-2xl">
        ...
        <p className="mt-1 text-sm text-navy-500">
          Validasi lokasi GPS terhadap dua titik kantor BPOM Jember (radius {kantor?.radius_meter ?? 100} m).
        </p>
                  ...
                  <div className="mt-3 rounded-xl bg-navy-50 px-4 py-3 text-sm">
                    ...
                    {wfhLokasi?.status === "diajukan" && (
                      <p className="text-amber-600">
                        Lokasi WFH Anda masih <strong>menunggu verifikasi</strong> tim IT/admin.
                      </p>
                    )}
                    ...
                    {!wfhLokasi && (
                      <p className="text-navy-600">Anda belum mendaftarkan lokasi WFH.</p>
                    )}
                    ...
                    {showWfhForm && (
                      <form onSubmit={handleWfhSubmit} className="mt-3 flex flex-col gap-3">
                        <Input
                          label="Label"
                          value={wfhForm.label}
                          onChange={(e) => setWfhForm((f) => ({ ...f, label: e.target.value }))}
                        />
                        <Textarea
                        ...
                        <Button type="submit" size="sm" loading={wfhSubmitting}>
                          Ajukan Lokasi WFH
                        </Button>
                      </form>
                    )}
                  ...
                  <div key={r.id} className="flex items-center justify-between py-3">
                    <div>
                      ...
                      <p className="mt-0.5 flex items-center gap-1.5 text-xs text-navy-400">
                        ...
                        {r.mode_masuk && (
                          <span className="uppercase tracking-wide">• {r.mode_masuk}</span>
                        )}
                      </p>
                    </div>
